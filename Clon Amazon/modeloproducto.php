...
<a name="arriba"></a>
    ...
    <header>
        ...
        <ul>
                    <li>
                        <form action="busquedaart.php" method="GER">
        ...
        <?php
            $v1 = $_POST['codigoproducto'];
            $consulta = $conexion->query("SELECT * FROM productos WHERE id LIKE '%$v1%'");
            $row=$consulta->fetch_array();
        ?>

    <div align="center" class="contcardsProducto">
            <div class="itemcardImagenProducto">
                <?php
                    echo '<img style="width: 350px; height: 300px;" src="data:image/jpeg;base64,'.base64_encode($row["imagen"]).'"/>';
                ?>
            </div>

            <div class="itemcardDescripcionProducto">
                <?php
            ...
            <div class="itemcardInformProduct">
                <?php 
                   echo '<h2> $ '.$row["precio"].'</h2>';?>
                   <div class="fecha">
                    <br>
                    <h4>Entrega Gratis <script type="text/javascript"> var meses = new Array ("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"); var diasSemana = new Array("Domingo","Lunes","Martes","Miércoles","Jueves","Viernes","Sábado"); var f=new Date(); document.write(diasSemana[f.getDay()+1] + ", " + (f.getDate()+1) + " de " + meses[f.getMonth()]);</script></h4>
                   </div>
                   <br>
                   ...
                   <br><br>
                   <form action="underconst.php" method="post">
                        <input class="botonproduct1" style="background-color: #ffd814" type="submit" value="Agregar al Carrito">
                    </form>
                    <br>
                    <form action="underconst.php" method="post">
                        <input class="botonproduct1" style="background-color: #ffa41c"type="submit" value="Comprar ya">
                    </form>
                    <br>
                    <h4><img align="left" src="imagenes\bloquear.png">Transacción Segura</h4>
                    <br>
                   <?php
                    echo '<p>Vendido Por: '.$row["Vendido"].'</p>';
                    echo '<p>Enviado Por: '.$row["Envio"].'</p>';
                ?>
            </div>
    </div>
